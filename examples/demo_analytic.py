...
class DemoAnalytic(object):
    def __init__(self, co, nu, su, zh_mid, Mmin_hiMsun, Mmax_hiMsun, rp_min_hiMpc, rp_max_hiMpc, n_rp):
        ...
        ## Do I nneed descimal points for the dir name?
        m1, x = np.modf(Mmin_hiMsun/(1e+14))
        m2, x = np.modf(Mmax_hiMsun/(1e+14))
        # if m1 > 1e-4 or m2 > 1e-4:
        #     print('%.2e %.2e'%(Mmin_hiMsun, Mmax_hiMsun))
        # else:
        #     print('no decimal')
        #     print('%.e %.e'%(Mmin_hiMsun, Mmax_hiMsun))
        # exit()
        if m1 > 1e-4 or m2 > 1e-4:
            self.output_loc = '../output/analytic_abacus_scatter%s/zh%g_zs%g/%.2e_%.2e_R%g_%g_nrp%i/'%(0, zh_mid, self.zs_mid, Mmin_hiMsun, Mmax_hiMsun, rp_min_hiMpc, rp_max_hiMpc, n_rp)
        else:
...
if __name__ == "__main__":
    ## ./demo_analytic.py 0.5 1.25 1e+14 1e16 2 0
    ## ./demo_analytic.py 0.3 0.75 2e+14 1e16 2 0
