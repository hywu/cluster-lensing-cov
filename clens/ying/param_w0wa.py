...
defaultCosmoParamDict = {
   ...
   # ``w`` is kept for backward compatibility with the old constant-w
   # interface.  The CPL/w0wa form is
   #
   #     w(a) = w0 + wa * (1 - a)
   #
   # Lambda-CDM is recovered for w0 = -1 and wa = 0.
   'w'              : -1.0,
   'w0'             : -1.0,
   'wa'             : 0.0,
   'tau'            : 0.088,
...
class CosmoParams(dict):
    ...
    **pardict: dict_like
    ...
    def __init__(self, set_flat=False, **pardict):
        ...
        # Accept several common names for CPL/w0wa dark energy.  The
        # density_w0wa.py and frw_w0wa.py modules look for these aliases;
        # internally we store the canonical names ``w0`` and ``wa``.
        param_aliases = {
            'w_0': 'w0',
            'omega_w_0': 'w0',
            'dark_energy_w0': 'w0',
        ...
        #   * Old code may pass/use ``w`` for constant-w CDM.
        #   * New code should pass/use ``w0`` and ``wa`` for CPL/w0waCDM.
        # If only ``w`` is explicitly supplied, interpret it as w0.
        # Otherwise keep ``w`` synchronized to the canonical present-day w0.
        if ('w' in pardict) and not any(k in pardict for k in
                                        ['w0', 'w_0', 'omega_w_0',
                                         'dark_energy_w0']):
            self['w0'] = self['w']
        self['w'] = self['w0']
    ...
    def __getattr__(self,name):
        ...
        key = param_aliases.get(name, name)
        try:
            return(self[key])
        except KeyError:
    ...
    def isw0waCDM(self):
        """Tell if the cosmology uses non-Lambda CPL/w0wa dark energy.

        Returns
        -------
        isw0waCDM: boolean
            True if w0 != -1 or wa != 0.
        """
        return((abs(self["w0"] + 1.0) > self.tolerance) or
               (abs(self["wa"]) > self.tolerance))

    def isLambdaCDM(self):
        """Tell if the dark-energy equation of state is Lambda-like.

        This only checks w0 and wa, not spatial curvature.
        """
        return(not self.isw0waCDM())
