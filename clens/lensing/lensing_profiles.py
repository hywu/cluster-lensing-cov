...
class LensingProfiles(object):
    def __init__(self, co, nu, su, zh_min, zh_max, Mmin, Mmax):
        self.co = co
        self.nu = nu
        self.su = su
    ...
    def calc_DeltaSigma(self):
        ...
        self.cf3d = CorrelationFunctions3D(z=zh, co=self.co, nu=self.nu, lambda_min=self.Mmin, lambda_max=self.Mmax)
        r = self.cf3d.radius
        xi_cm = self.cf3d.xi_cm()
        rp, DeltaSigma, xi, ximean = DeltaSigmaR(r, xi_cm, rp_max=1000, rho=self.rho_mean) # Ying's code
...
if __name__ == "__main__":
    ...
    lp = LensingProfiles(co=co, nu=nu, su=su, zh_min=0.155, zh_max=0.323, Mmin=1e+14, Mmax=1e+16)
