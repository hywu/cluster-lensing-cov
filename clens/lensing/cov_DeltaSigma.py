...
class CovDeltaSigma(object):
    ...
    """
    def __init__(self, co, nu, su, fsky=1, slicing=False, dz_slicing=0, halo_shot_noise_only=False, cosmic_shear_no_shot=False):
        """
        Args:
    ...
    def calc_mean(self, Mmin, Mmax, zh_min, zh_max, rp_min=0.1, rp_max=100):
        lp = LensingProfiles(co=self.co, nu=self.nu, su=self.su, zh_min=zh_min, zh_max=zh_max, Mmin=Mmin, Mmax=Mmax)
    ...
    def calc_cov(self, rp_min, rp_max, n_rp, zh_min, zh_max, Mmin, Mmax, diag_only=False):
        ...
        if diag_only==True: ## only calculating the diagonal elements
            for ith1 in range(nth):
                ith2 = ith1
                self._calc_C_ell_integration(thmin1=thmin_list[ith1], thmax1=thmax_list[ith1], thmin2=thmin_list[ith2], thmax2=thmax_list[ith2], zh_min=zh_min, zh_max=zh_max, Mmin=Mmin, Mmax=Mmax)
...
def demo_var_slicing():
    ...
    cds.calc_cov(rp_min=rp_min, rp_max=rp_max, n_rp=n_rp, zh_min=0.1, zh_max=0.3, Mmin=1e14, Mmax=1e16, diag_only=True)
