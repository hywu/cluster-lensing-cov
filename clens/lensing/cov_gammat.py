...
class Covgammat(object):
    ...
    """
    def __init__(self, co, su, sr, fsky, slicing=False, dz_slicing=0, no_halo=False, halo_shot_noise_only=False, calc_kappa=False):
        """
        Args:
            co: cosmology object
    ...
    def _calc_C_ell_integration(self, thmin1, thmax1, thmin2, thmax2, zh_min, zh_max, lambda_min, lambda_max):
        ...
        Args:
            ...
            lambda_min (float): min mass of halos, in Msun (no h)
            lambda_min (float): max mass of halos, in Msun (no h)
        """
        ...
        if self.no_halo == False: ## calculating the contribution from halo counts: C_ell_h and shot noise
            ...
            #self.aps.calc_C_ell_h_kappa(zh_min=zh_min, zh_max=zh_max, lambda_min=lambda_min, lambda_max=lambda_max)
        ...
        #print('compare integration')
        #print('cosmic shear', self.cosmic_shear_integration)
        #print('intrinsic', self.halo_intrinsic_integration)
        


    def calc_cov_gammat_integration(self, thmin, thmax, nth, zh_min, zh_max, lambda_min, lambda_max, diag_only=False):
        ...
        if diag_only==True: ## only calculating the diagonal elements
            for ith1 in range(nth):
                ith2 = ith1
                self._calc_C_ell_integration(thmin1=self.thmin_list[ith1], thmax1=self.thmax_list[ith1], thmin2=self.thmin_list[ith2], thmax2=self.thmax_list[ith2], zh_min=zh_min, zh_max=zh_max, lambda_min=lambda_min, lambda_max=lambda_max)
...
def demo_var():
    ...
    su = Survey()
    fsky = 5000. / 41253.
    cj = Covgammat(co=co, su=su, sr=sr, fsky=fsky)
    thmin = 5e-4
    thmax = 5e-2
    nth = 2#10#4
    cj.calc_cov_gammat_integration(thmin=thmin, thmax=thmax, nth=nth, zh_min=0.1, zh_max=0.3, lambda_min=20, lambda_max=100, diag_only=True)
    plt.figure()
...
def demo_var_slicing():
    ...
    su = Survey()
    fsky = 5000. / 41253.
    cj = Covgammat(co=co, su=su, sr=sr, fsky=fsky, slicing=True, dz_slicing=0.03)
    thmin = 5e-4
    thmax = 5e-2
    nth = 2#10#4
    cj.calc_cov_gammat_integration(thmin=thmin, thmax=thmax, nth=nth, zh_min=0.1, zh_max=0.3, lambda_min=20, lambda_max=100, diag_only=True)
...
def demo_cov(save_files=False):
    ...
    lambda_min = 20
    lambda_max = 100
    cj.calc_cov_gammat_integration(thmin=thmin, thmax=thmax, nth=nth, zh_min=zh_min, zh_max=zh_max, lambda_min=lambda_min, lambda_max=lambda_max)
    ...
    if save_files==True:
        ...
        outfile = open(output_loc+'var_cosmic_shear_fullsky_z_%g_%g_M_%.0e_%.0e.dat'%(zh_min,zh_max,lambda_min,lambda_max),'w')
        ...
        outfile = open(output_loc+'var_shape_noise_fullsky_z_%g_%g_M_%.0e_%.0e.dat'%(zh_min,zh_max,lambda_min,lambda_max),'w')
        outfile.write('# theta[radian], var gammat \n')
        for it in range(len(cj.thmid_list)):
        ...
        ## save the entire cov
        outfile = open(output_loc+'cov_sum_fullsky_z_%g_%g_M_%.0e_%.0e.dat'%(zh_min,zh_max,lambda_min,lambda_max),'w')
